...
namespace Team2_Project_DAO
{
    public class DashboardDAO : IDisposable
    {
        ...
        public List<TimeProductionHistoryDTO> Production() //생산진행현황 (작업지시별)
        {
            try
            {
                using (SqlCommand cmd = new SqlCommand("SP_GetDashBoardTimeProductionHistory", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    conn.Open();
        ...
        public List<TimeProductionHistoryDTO> GetProductionHistory() //생산실적현황 (시간대별)
        {
            try
            {
                string sql = @"SELECT TOP 10 WorkOrderNo, Start_Hour, In_Qty_Main, Out_Qty_Main, Prd_Qty, Prd_Unit, CONVERT(VARCHAR(23), Ins_Date, 21) Ins_Date 
